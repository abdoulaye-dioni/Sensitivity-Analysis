**Sensitivity Analysis in Non-Hierarchical Models When Extreme Categories of Ordinal Variables Are Missing** _(Senitivity_analysis_NH_EC.md)_
...
* `y`: a binary response such as $$ P(Y = 1|X) = \frac{\exp(X\beta)}{1 + \exp(X\beta)}$$. It is generated by sampling from a binomial distribution based on the probabilities of outcomes `0 and 1`.
